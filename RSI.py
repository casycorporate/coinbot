...
ax1.plot(eth_usdt['close'], linewidth=2.5)
ax1.set_title('IBM CLOSE PRICE')
ax2.plot(eth_usdt['rsi_14'], color='orange', linewidth=2.5)
...
buy_price, sell_price, rsi_signal = implement_rsi_strategy(eth_usdt['close'], eth_usdt['rsi_14'])

ax1 = plt.subplot2grid((10, 1), (0, 0), rowspan=4, colspan=1)
ax2 = plt.subplot2grid((10, 1), (5, 0), rowspan=4, colspan=1)
ax1.plot(eth_usdt['close'], linewidth=2.5, color='skyblue', label='IBM')
ax1.plot(eth_usdt.index, buy_price, marker='^', markersize=10, color='green', label='BUY SIGNAL')
ax1.plot(eth_usdt.index, sell_price, marker='v', markersize=10, color='r', label='SELL SIGNAL')
...
rsi_signal = pd.DataFrame(rsi_signal).rename(columns={0: 'rsi_signal'}).set_index(eth_usdt.index)
position = pd.DataFrame(position).rename(columns={0: 'rsi_position'}).set_index(eth_usdt.index)
...
ibm_ret = pd.DataFrame(np.diff(eth_usdt['close'])).rename(columns={0: 'returns'})
rsi_strategy_ret = []
...
number_of_stocks = floor(investment_value / eth_usdt['close'][-1])
rsi_investment_ret = []
